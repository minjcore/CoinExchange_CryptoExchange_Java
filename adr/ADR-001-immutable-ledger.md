...
| ID | Criterion | Owner module |
|----|-----------|--------------|
| AC-001-01 | After `coa_trans.status = POSTED`, no UPDATE changes `amount`, `dr_cr`, or `coa_account_id` on existing `coa_trans_data` rows | `core.accounting` |
| AC-001-02 | Corrections to POSTED journals use a **new** `coa_trans` with reversal/mirror lines; `reverses_id` links to source | `core.accounting` |
| AC-001-03 | Trial balance and control accounts derive from posted lines; no authoritative `account_balance` write instead of a posting | `core.accounting` |
| AC-001-04 | `createJournal` + `post` in one ACID TX; post rejected if DR ≠ CR or period closed | `core.accounting` |
| AC-001-05 | Duplicate `business_ref` + `use_case` returns existing `coa_trans_id` without second post | `core.accounting` |
| AC-001-06 | Wallet credit for deposit occurs only after POSTED — wallet never reads `coa_*` for idempotency | `core.wallet` / orchestration |
| AC-001-07 | PENDING deposit holds transit (e.g. **3100**); POSTED clears transit to zero for completed use case | `core.accounting` |
...
|----|-------|---------------------|----------|---------|
| TC-001-01 | Posted lines immutable | POST deposit POSTED; attempt UPDATE `coa_trans_data` row | DB constraint or service reject | AC-001-01 |
| TC-001-02 | Reversal not in-place edit | POST `orig-1`; ops reversal | New journal + `reverses_id`; original rows unchanged | `acceptance.md` ADJ-E01, X-E04 |
| TC-001-03 | Unbalanced post rejected | Journal DR ≠ CR | `ACCOUNTING_UNBALANCED_JOURNAL`; no POSTED | `accounting.md` §8 |
| TC-001-04 | Period close blocks post | Post with `posting_date` in closed period | `ACCOUNTING_PERIOD_CLOSED` | `acceptance.md` X-E03 |
| TC-001-05 | Duplicate journal ref | `createJournal` same `reference_id` + `use_case` twice | Same `coa_trans_id`; one effect | `acceptance.md` ledger duplicate ref |
| TC-001-06 | Deposit two-phase transit | Phase A PENDING then phase B POSTED | 3100 net zero; wallet credited only after POSTED | `acceptance.md` Deposit happy + Phase A/B |
| TC-001-06b | Wallet not credited on PENDING | Phase A only | `wallet.available` unchanged | `acceptance.md` Phase A only |
| TC-001-07 | PENDING reverse scope | Reverse PENDING deposit A | Only 1111/3100 reversed; no 2110 credit | `acceptance.md` ADJ-E03 |
