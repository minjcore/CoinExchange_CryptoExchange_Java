...
| ID | Criterion | Surfaces |
|----|-----------|----------|
| AC-005-01 | S1 `X-Idempotency-Key` propagates unchanged as `businessRef` / `business_ref` through S2, S3, S6, wallet, accounting | S1–S6 |
| AC-005-02 | Wallet: `UNIQUE (wallet_id, business_ref, tx_type)`; replay returns same `wallet_tx_id` without balance effect | `core.wallet` |
| AC-005-03 | Same triple + **different amount** → `WALLET_DUPLICATE_CONFLICT` / HTTP 409 | `core.wallet` |
| AC-005-04 | Payment D2: same `businessRef`, `PAYMENT_DEBIT` + `PAYMENT_CREDIT` on different wallets | orchestration |
| AC-005-05 | Transfer D3: same `businessRef`, `TRANSFER_DEBIT` / `TRANSFER_CREDIT` | orchestration |
| AC-005-06 | Withdraw/IBFT D5: sub-keys `{ref}:settle`, `{ref}:release` for follow-up legs | orchestration |
| AC-005-07 | Accounting: `UNIQUE (reference_id, use_case)` on `coa_trans` | `core.accounting` |
| AC-005-08 | S6 `messageId` dedups transport only — never used as business idempotency key | S6 consumers |
| AC-005-09 | `coaTransId` on `wallet_tx` is correlation only — not idempotency, no cross-schema FK | wallet + orchestration |
| AC-005-10 | Mutating S1 without idempotency key → 400 | S1 |
...
|----|-------|---------------------|----------|---------|
| TC-005-01 | End-to-end key propagation | Payment with key `pay-1` | Same ref on S2, wallet_tx, events | `orchestration.md` §8 |
| TC-005-02 | Wallet replay same triple | Retry `DEPOSIT_CREDIT` same ref | Idempotent 200; balance unchanged | `acceptance.md` Deposit idempotent |
| TC-005-03 | Amount conflict 409 | Same ref, amount 100000 then 90000 | `WALLET_DUPLICATE_CONFLICT` | `acceptance.md` Idempotency conflicts |
| TC-005-04 | Payment two legs one ref | `PAYMENT_DEBIT` + `PAYMENT_CREDIT` same `businessRef` | Both succeed | `acceptance.md` Payment two legs |
| TC-005-05 | Transfer two legs one ref | `TRANSFER_DEBIT` + `TRANSFER_CREDIT` | Both succeed | `acceptance.md` Transfer |
| TC-005-06 | Withdraw sub-keys | SETTLE with `{ref}:settle` twice | Idempotent; frozen debited once | `acceptance.md` Withdraw SETTLE idempotent |
| TC-005-07 | IBFT sub-keys | SETTLE replay `ibft-4` | `acceptance.md` IBFT SETTLE idempotent | IBFT feature |
| TC-005-08 | Accounting duplicate ref | Same `reference_id` + `use_case` | Existing `coa_trans_id` | `acceptance.md` ledger duplicate ref |
| TC-005-09 | messageId ≠ businessRef | Redeliver S6 same `businessRef`, new `messageId` | One business effect | `acceptance.md` X-E07 |
| TC-005-10 | Missing idempotency key | POST payment without header | 400 | `acceptance.md` X-E02 |
| TC-005-11 | use_case clash | DEPOSIT ref reused for PAYMENT journal | Accounting rejects | `acceptance.md` ledger same ref different use_case |
...
- [`design-v2/orchestration.md`](../design-v2/orchestration.md) — §8 propagation, §14.4 / §15.7 sub-keys
- [`design-v2/wallet.md`](../design-v2/wallet.md) — §3 D1–D5
- [`design-v2/acceptance.md`](../design-v2/acceptance.md) — Idempotency, X-E features
