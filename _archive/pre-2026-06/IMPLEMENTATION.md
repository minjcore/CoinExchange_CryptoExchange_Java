...
│       ├── web/                     S1 controllers, GlobalExceptionHandler
│       ├── accounting/              AccountingClient (S2) or in-process facade
│       ├── wallet/                    WalletFacade (thin — maps DTO → commands)
│       ├── messaging/                 S6 publisher, S3 listeners (phase P5+)
│       └── usecase/                 PaymentUseCase, DepositUseCase, …
└── app-wallet-worker/               optional P5+
    └── messaging/                   S6 WalletCreditConsumer
```
...
| `app-wallet-worker` | wallet | Boot + AMQP | wallet only | wallet | ✓ |
...
These were open in [`core.wallet.md`](./core.wallet.md) §3.4 / §5.2 — fixed here for migrations and orchestration.
...
v1 uses **freeze** on accept, not immediate debit.
...
Minimal v1 — extend per [`core.accounting.trd.md`](./core.accounting.trd.md).
...
### 9.1 S1 → orchestration classes

| OpenAPI operationId | Class | Delegates to |
|---------------------|-------|--------------|
| `getWalletBalance` | `WalletController` | `WalletQueryService.getBalance` |
| `createPayment` | `PaymentController` | `PaymentUseCase.execute` |
| `createTransfer` | `TransferController` | `TransferUseCase.execute` |
| `createWithdrawal` | `WithdrawController` | `WithdrawUseCase.execute` |
| `notifyDeposit` / `bankWebhook` | `DepositController` | `DepositUseCase.acceptAsync` → S6 publish |

Wrap responses: `ApiResponse.ok(data)`; map `BaseException` in `GlobalExceptionHandler`:
...
- [ ] Flyway V1 wallet schema
- [ ] JPA entities + repositories
- [ ] `WalletCommandServiceImpl` with algorithm §6.4
- [ ] Testcontainers integration tests §6.6
- [ ] Spring `@Configuration` for wallet EMF (used by app later)

### P2 — accounting domain (3–5 days)

- [ ] Flyway V1 + V2 seed COA
- [ ] `JournalServiceImpl` — create / addLines / post / idempotency
- [ ] Deposit two-phase helper (PENDING + POSTED lines)
- [ ] Payment line template §7.4
- [ ] Tests §7.6

### P3 — first HTTP slice (2–3 days)

- [ ] `app-orchestration` Boot app, dual datasource
- [ ] `GET /v1/wallets/balance` → OpenAPI shape
- [ ] Auth stub (JWT parse `memberId` from claims — detail in gateway)
- [ ] Actuator health + DB

### P4 — sync payment (3–4 days)

- [ ] `POST /v1/payments` full D2 + §7.4 + §8.2
- [ ] Idempotency header → `businessRef`
- [ ] IT: pay → balances; duplicate header → replay
