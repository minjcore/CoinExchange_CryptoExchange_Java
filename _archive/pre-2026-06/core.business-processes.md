...
### 3.3 Wallet effect ([`core.wallet.md`](./core.wallet.md) §5.1)
...
### 4.3 Wallet effect ([`core.wallet.md`](./core.wallet.md) §5.4)
...
### 5.3 Wallet effect ([`core.wallet.md`](./core.wallet.md) §5.2)
...
Same `businessRef`, one distinct `tx_type` per leg. Insufficient funds → reject, no partial debit ([`core.wallet.md`](./core.wallet.md) FR-4).
...
### 6.2 Wallet effect ([`core.wallet.md`](./core.wallet.md) §5.3)
...
> Wallet: no per-transaction `wallet_tx` for acquirer flow until the product decides to credit the merchant wallet ([`core.wallet.md`](./core.wallet.md) §5.6).
...
### 10.2 Wallet effect ([`core.wallet.md`](./core.wallet.md) §5.5)
...
> Control principle: every transit returns to **0** on completion; total `wallet_balance` per lane reconciles to control **2110+2120+2130** ([`core.wallet.md`](./core.wallet.md) §6 W5).
...
> These are stated in domain terms (steps, states, keys) so the design survives a language/runtime change (e.g. Java → Go): only the wire (`open-api/`, `async-api/`) and idempotency keys are contractual; the recovery semantics above are the spec.
...
| Partner / bank channel | mTLS (`notifyDeposit`, `bankWebhook`) — see `open-api/gtelpay-public.yaml` |
...
| Internal accounting API (S2) | Service-to-service auth; RBAC roles from `core.accounting.trd.md` NFR-6: Accountant, Auditor, Admin, ReadOnly |
