...
**Not in foundation (v1):** Duplicate command/event DTOs, port interfaces, Kafka/HTTP adapters. Wire shapes live in **`spec/contracts/open-api/`** and **`spec/contracts/async-api/`**; Application maps JSON → domain service method parameters.
...
| OpenAPI / AsyncAPI YAML | `spec/contracts/open-api/`, `spec/contracts/async-api/` (wire source of truth) |
...
| `coa_trans` | **Journal** — balanced DR/CR header: `use_case`, `reference_id`, `status`, timestamp |
...
| `coa_trans` | **Journal** — one balanced posting header: use_case, reference_id, time, status (total DR = total CR of lines) |
...
|----------------|------------|
| `coa_trans.reference_id` | **UNIQUE** per `use_case` (or globally per product rule) — idempotency |
...
|---------------|-------|-------------------|
| **Journal** | **`coa_trans`** | One balanced DR/CR event (deposit, withdraw, …); `use_case`, `reference_id`, `status` |
...
**ADR:** [ADR-006 — Two-phase deposit](./adr/ADR-006-two-phase-deposit.md) · [ADR-010 — Transit zero](./adr/ADR-010-transit-accounts-net-zero.md).
...
| Idempotent | `reference_id` = bank txn id — duplicate webhook/confirm does not double-post (**UNIQUE**) |
